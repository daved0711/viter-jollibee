...
const DashboardAccordion = ({title,filterby}) => {
  ...
  return (
      ...
      <div className='accordion mb-2'>
                        <div className="accordion-header p-2 flex justify-between bg-secondary rounded-t-md cursor-pointer" onClick={handleToggleOpen}>
                          <h6 className='mb-0'>{title}</h6>
                         
                            <ChevronDown className={`transition-all  ${isOpen ? "rotate-180" : ""}`} />
                         
                         </div>
                        <div className={`accordion-body  border border-line rounded-b-md border-t-0 overflow-hidden ${ isOpen ? "max-h-[600px]" : "max-h-[0px]"}`}>
